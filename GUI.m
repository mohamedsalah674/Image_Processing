...
% Last Modified by GUIDE v2.5 19-Dec-2022 21:02:34
...
data = get(handles.slider_Brightness,'Value');
...
rest = getappdata(0,'filename');
Restore = imrest(rest,'hmean',3,3);
axes(handles.axes2);
imshow(Restore)
setappdata(0,'filename',Restore)
...
setappdata(0,'filename',min_Image)
...
setappdata(0,'filename',max_Image)
...
% --- Executes on button press in Contraharmonic_meanP.
function Contraharmonic_meanP_Callback(hObject, eventdata, handles)
 rest = getappdata(0,'filename');
...
% --- Executes on button press in Contraharmonic_meanN.
function Contraharmonic_meanN_Callback(hObject, eventdata, handles)
 rest = getappdata(0,'filename');
 Restore=imrest(rest,'chmean',3,3,-2);
 axes(handles.axes2);
 imshow(Restore)
...
% Hint: get(hObject,'Value') returns toggle state of Geometric_mean
